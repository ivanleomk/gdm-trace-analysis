...
30-45 minute writeup. The candidate reads ~20 support-agent traces plus one customer note, then names loss patterns a modeling team can hook.
...
4. Open [`packet/traces.jsonl`](packet/traces.jsonl). 20 sessions, one JSON object per line. `tags_gold` is empty.
